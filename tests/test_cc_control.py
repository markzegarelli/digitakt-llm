...
def test_repl_cc_valid(tmp_path):
    from cli.main import _run_repl
    state = AppState()
    bus = EventBus()
    mock_port = MagicMock()
    player = Player(state, bus, mock_port)
    ...
    commands = iter(["cc kick filter 90", "exit_sentinel"])
    ...
    try:
        with pytest.raises(SystemExit):
            _run_repl(player, gen, state, mock_port)
    finally:
        builtins.input = original_input

    assert state.track_cc["kick"]["filter"] == 90


def test_repl_cc_invalid_prints_usage(tmp_path, capsys):
    from cli.main import _run_repl
    state = AppState()
    bus = EventBus()
    mock_port = MagicMock()
    ...
    commands = iter(["cc badtrack filter 64"])
