...
const HELP_LINES = [
  ...
  "  mute <track> [on|off|toggle]         queue mute at next bar (default: toggle)",
  ...
  "  chain <p1> <p2> ... [--auto]         define setlist; optional auto-advance each bar",
  "  chain next                           queue next pattern in chain (does not switch yet)",
  "  chain fire                           arm queued chain pattern for next downbeat (next 1)",
  "  chain fill <slot>                    one-shot fill from chain slot 1…n (rejected if fill playing)",
  "  chain status                         chain position, queued slot, armed state",
  ...
  "  midi                                 connect MIDI (auto-detect Digitakt); flush CC + start if playing",
  "  midi list                            list MIDI output port names",
  "",
  "── Chat & Generation ──────────────────────────────────────────",
  "  <bare text>                          generate beat (BEAT mode) or ask Claude (CHAT mode)",
  ...
  "  mode [chat|beat|standard|euclidean]  input mode (chat/beat) or seq mode (standard/euclidean)",
  ...
  "                                       (SEQ browse or MIX; not during step/TRIG/CC-step edit)",
  "  c / n / Shift+N                      if chain exists: focus strip / queue next / arm fire on next 1",
  "  Shift+!@\u0023$%^&*(                  chain slot 1-9 fill (chain active; not TRIG digit mode)",
  "  f then 1-9 (within ~0.8s)            same (layout-neutral; not while CMD focused)",
  "",
  ...
  "  euclid-strip [grid|fractional]       strip layout; bare toggles in Euclidean only",
  ...
  "  ring note: 16-dot perimeter uses +15 slot rotation (logical step 1 at top)",
  "",
  "TRIG panel",
  ...
  "  i                                    jump focus here from SEQ or CMD",
  "  m / q / Shift+Q                      mute selected track (same as SEQ browse)",
  "  ↑/↓                                  select CC parameter",
  "  [ ]                                  select track",
  "  ←/→                                  adjust global CC (Shift+←/→ = ±10)",
  "  l                                    open LFO editor for selected CC param",
  "  Enter / e                            enter per-step CC edit",
  ...
  "  M/Q badges on track rows: muted / queued mute (MQ = both)",
  "",
  ...
  return (
    <Box flexDirection="column" borderStyle="single" borderColor={borderCol} paddingX={1}>
      ...
      {statusLine
        ...
        : <Text color={theme.textFaint}>{"? Help  Tab/i Focus  m/q/Q Mute  Shift+Tab Mode  / Cmd  t TRIG  Space Play/Stop"}</Text>
      }
