...
**Spec:** `docs/specs/2026-04-12-v1-design.md` — optional design doc (directory may be gitignored). **Acceptance criteria and ordered gates for v1:** [docs/ROADMAP.md](../ROADMAP.md).

### Status vs this repository (refactored milestones)

| Planned milestone | In tree | Notes |
|-------------------|---------|-------|
| v0.2 Chain engine | **Done** (superset) | `AppState` holds chain names, embedded patterns, index, auto, queued slot/pattern, armed flag. REST: `POST /chain`, `POST /chain/next`, `POST /chain/fire`, `DELETE /chain`. Player emits `chain_advanced` and related events — see `ARCHITECTURE.md`. |
| v0.3 Instrument panel | **Done** | `StatusBar`, `ChainPanel`, `StepGrid`, `GenerationSummary` in `tui/src/App.tsx`; `generation_complete` includes `summary`. |
| v0.4 Feedback loop as `/next`, `/vary`, `/read` | **Superseded** | Implemented surface: `POST /generate` + `variation`, `POST /ask`, TUI `/fresh`, `/gen`, mode switching. Tasks 14–17 below describe an **alternate** API that was not shipped; see `docs/ROADMAP.md` Gate A. |
| v1.0 Integration / docs | **Partial** | Task 18 rewritten below to match shipped commands and `ARCHITECTURE.md`. |

---

## File Map

> Original task-to-file map for the first implementation pass. Completion status is summarized in the **Status vs this repository** table above; chain and UI code have diverged in places (e.g. extra chain fields, no `GET /chain`).
...
### Task 18: Documentation and CLAUDE.md update (aligned to shipped code)

**Files:**
- Modify: `CLAUDE.md` — slash commands, keyboard shortcuts, REST summary (must match `tui/src/App.tsx` and `api/server.py`).
- Modify: `ARCHITECTURE.md` — every `POST`/`GET`/`DELETE` in `api/server.py`, WebSocket events actually broadcast, `AppState` fields for chain + playback.
- Optional: `VISION.md` — only if you maintain a tracked copy; it is gitignored in this repo by default.

- [ ] **Step 1: Verify CLAUDE.md matches the TUI**

Confirm the UI section documents at least: chain subcommands (`/chain`, `/chain next`, `/chain fire`, `/chain status`, `/chain clear`), beat/chat modes, `/ask`, `/fresh`, `/gen`, pattern/fill/load/save, TRIG/SEQ behavior as implemented in `tui/src/App.tsx` and `tui/src/components/Prompt.tsx`.

- [ ] **Step 2: Purge references to unimplemented feedback endpoints**

Do **not** document `POST /next`, `POST /vary`, `POST /read`, or `GET /chain` unless they are added to `api/server.py`. The feedback loop for v1 is: `POST /generate` (optional `variation`), `POST /ask`, and the TUI helpers above.

- [ ] **Step 3: ARCHITECTURE.md parity pass**

Ensure tables list real routes (e.g. `POST /chain/fire`, `POST /ask`, `GET /traces` when enabled) and chain-related events (`chain_updated`, `chain_queued`, `chain_armed`, `chain_advanced`) with payloads consistent with `api/server.py` / `core/player.py`.

- [ ] **Step 4: Cross-link the roadmap**

Add or verify a pointer to [docs/ROADMAP.md](../ROADMAP.md) from `CLAUDE.md` so release gates stay visible.
...
```bash
git add CLAUDE.md ARCHITECTURE.md docs/ROADMAP.md
git commit -m "docs: align references with shipped API for v1.0.0"
```
...
| `build_context_prompt()` | Task 14 — **superseded** unless re-scoping to match `Generator` today |
| `/next [section]` | Tasks 15, 16, 17 — **not shipped**; use `POST /generate` + context / `/fresh` instead |
| `/vary [intensity]` | Tasks 15, 16, 17 — **not shipped**; use `POST /generate` + `variation` instead |
| `/read` | Tasks 15, 16, 17 — **not shipped**; use `POST /ask` for prose about the session |
| Docs updated | Task 18 (revised above) |

**Plan vs tree:** Original Tasks 14–17 assumed `NextRequest` / `VaryRequest` / `ReadResponse` schemas — these types are **not** in the codebase. Chain payloads evolved (queued index, armed flag, embedded patterns). Prefer [docs/ROADMAP.md](../ROADMAP.md) for v1 ordering.

**Type consistency (historical):** Where implemented, `ChainRequest` and chain fields follow Pydantic / `AppState` naming; validate any new public API the same way.
